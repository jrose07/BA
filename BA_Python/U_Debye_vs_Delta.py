...
for i, u in enumerate(U):
    for j, e_d in enumerate(E_D):
        deltas[j, i] = get_delta(u, T=T, E=E, E_debye=e_d, n_fixpunkt=20, start=1)
...
mesh_ = ax.imshow(deltas, extent=[U.min(), U.max(), E_D.min(), E_D.max()], origin='lower', aspect='auto', cmap='inferno')
