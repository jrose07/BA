...
def newton_func(delta, U, T, E_D, mu, num_points):
    ...
    if T == 0:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            arg = E_k / (2 * (k_b/const.e) * T ) #T in Einheit K * t/eV
...
def get_next_delta(delta, T, U, mu, E):
    ...
    if T==0:
        ...
        arg = E_k / (2 * (k_b/const.e) * T ) #T in Einheit K * t/eV
        integral = integrate.simpson(integrand * np.tanh(arg), x=E)
...
def get_delta_2(E, DOS, mu, start, T, U, iterations):
    ...
    for i in range(iterations):
        ...
        if T==0:
            ...
            arg = E_k / (2 * (k_b/const.e) * T ) #T in Einheit K * t/eV
            integral = integrate.simpson(DOS*deltas[i]/E_k * np.tanh(arg), x=E)
