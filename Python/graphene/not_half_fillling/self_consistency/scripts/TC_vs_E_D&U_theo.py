...
def get_T_C(U, mu, E_D):
    ...
    def T_C_scalar(U_val, E_D_val):
        T_C, _ = solve_Tc_exact(mu, E_D_val, U_val, A)
        return T_C
...
def main():
    ...
    T_C_df.to_csv(f"./data/TC_vs_E_D&U_theo_{version}.csv")
    ...
    colorbar = ax.contourf(t2mev(U)*1e-3, t2mev(E_D), T_C_masked, levels=levels, cmap='Spectral_r')
    ...
    fig.savefig(f"../plots/TC_vs_E_D&U_theo_{version}.pdf")
