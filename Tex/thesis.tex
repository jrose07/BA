%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%   Vorlage für eine Abschlussarbeit   %%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Erstellt von Maximilian Nöthe, <maximilian.noethe@tu-dortmund.de>
% ausgelegt für lualatex und Biblatex mit biber

% Kompilieren mit
% latexmk --lualatex --output-directory=build thesis.tex
% oder einfach mit:
% make

\documentclass[
  tucolor,       % remove for less green,
  % BCOR=12mm,     % 12mm binding corrections, adjust to fit your binding
  parskip=half,  % new paragraphs start with half line vertical space
  open=any,      % chapters start on both odd and even pages
  cleardoublepage=plain,  % no header/footer on blank pages
  DIV=12
]{tudothesis}


\RedeclareSectionCommand[
  beforeskip=10pt,
  afterskip=20pt
]{chapter}

% Does nothing i think
% \renewcommand{\topfraction}{0.9}
% \renewcommand{\bottomfraction}{0.8}
% \renewcommand{\textfraction}{0.07}
% \renewcommand{\floatpagefraction}{0.7}

% Warning, if another latex run is needed
\usepackage[aux]{rerunfilecheck}

% just list chapters and sections in the toc, not subsections or smaller
\setcounter{tocdepth}{1}

%------------------------------------------------------------------------------
%------------------------------ Fonts, Unicode, Language ----------------------
%------------------------------------------------------------------------------
\usepackage{fontspec}
\defaultfontfeatures{Ligatures=TeX}  % -- becomes en-dash etc.

% load english (for abstract) and ngerman language
% the main language has to come last
\usepackage[american, ngerman]{babel}

% intelligent quotation marks, language and nesting sensitive
\usepackage[autostyle]{csquotes}

% microtypographical features, makes the text look nicer on the small scale
\usepackage{microtype}

%------------------------------------------------------------------------------
%------------------------ Math Packages and settings --------------------------
%------------------------------------------------------------------------------

\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{mathtools}

% Enable Unicode-Math and follow the ISO-Standards for typesetting math
\usepackage[
  math-style=ISO,
  bold-style=ISO,
  sans-style=italic,
  nabla=upright,
  partial=upright,
  warnings-off={mathtools-colon,mathtools-overbracket}, % suppress some unnecessary warnings
]{unicode-math}
\setmathfont{Latin Modern Math}

% nice, small fracs for the text with \sfrac{}{}
\usepackage{xfrac}


%------------------------------------------------------------------------------
%---------------------------- Numbers and Units -------------------------------
%------------------------------------------------------------------------------

\usepackage[
  locale=DE,
  separate-uncertainty=true,
  per-mode=symbol-or-fraction,
]{siunitx}

%------------------------------------------------------------------------------
%-------------------------------- tables  -------------------------------------
%------------------------------------------------------------------------------

\usepackage{booktabs}       % \toprule, \midrule, \bottomrule, etc

%------------------------------------------------------------------------------
%-------------------------------- graphics -------------------------------------
%------------------------------------------------------------------------------

\usepackage{graphicx}
% currently broken
% \usepackage{grffile}

% allow figures to be placed in the running text by default:
\usepackage{scrhack}
\usepackage{float}
\floatplacement{figure}{htb}
\floatplacement{table}{htbp}

% \renewcommand{\topfraction}{0.9}
% \renewcommand{\textfraction}{0.07}
% \renewcommand{\floatpagefraction}{0.8}
% \setcounter{topnumber}{3}
% \setcounter{bottomnumber}{2}
% \setcounter{totalnumber}{4}




% keep figures and tables in the section
\usepackage[section, below]{placeins}

% allows to include PDFs as full pages
\usepackage{pdfpages}

% Set the PDF Version of this document to 1.7 (1.4 is the current default)
% This is needed so that PDFs with Version >1.5 can be included
\pdfvariable minorversion=7

\usepackage{wrapfig}
% 
\AtBeginDocument{\RenewCommandCopy\qty\SI}


% Better vectors
\usepackage[a]{esvect}
% \DeclareRobustCommand{\vec}[1]{\vv{#1}}

%------------------------------------------------------------------------------
%---------------------- customize list environments ---------------------------
%------------------------------------------------------------------------------

\usepackage{enumitem}

%------------------------------------------------------------------------------
%------------------------------ Bibliographie ---------------------------------
%------------------------------------------------------------------------------

\usepackage[
  backend=biber,   % use modern biber backend
  autolang=hyphen, % load hyphenation rules for if language of bibentry is not
                   % german, has to be loaded with \setotherlanguages
                   % in the references.bib use langid={en} for english sources
  sorting=none
]{biblatex}
\addbibresource{references.bib}  % the bib file to use
\DefineBibliographyStrings{german}{andothers = {{et\,al\adddot}}}  % replace u.a. with et al.



% Benes Biber

% \DeclareSourcemap{
%   \maps[datatype=bibtex]{
%     \map{
%       \step[
%         fieldsource=journal,
%         match={Nature Communications},
%         replace={Nat. Commun.}
%       ]
%     }
%     \map{
%       \step[
%         fieldsource=journal,
%         match={Physical Review Letters},
%         replace={Phys. Rev. Lett.}
%       ]
%     }
%     \map{
%       \step[
%         fieldsource=journal,
%         match={Physical Review},
%         replace={Phys. Rev.}
%       ]
%     }
%     \map{
%       \step[
%         fieldsource=journal,
%         match={The European Physical Journal},
%         replace={EPJ}
%       ]
%     }
%     \map{
%       \step[
%         fieldsource=journal,
%         match={MODELLING AND SIMULATION IN MATERIALS SCIENCE AND ENGINEERING},
%         replace={Model. Simul. Mater. Sci. Eng.}
%       ]
%     }
%   }
% }

% Quellendatenbank
% \addbibresource{reference.bib} 

% Formatierung der Felder anpassen
% \DeclareFieldFormat{title}{\mkbibbold{#1}}
% \DeclareFieldFormat{year}{\mkbibbold{#1}}
% \DeclareFieldFormat{doi}{\mkbibbold{DOI: #1}}

\DeclareFieldFormat{doi}{%
  % \mkbibbold{DOI:}%
  DOI:
  \addcolon\space
  \ifhyperref
    {\href{https://doi.org/#1}{\nolinkurl{#1}}} % Klickbarer Link
    {\nolinkurl{#1}} % Nur DOI anzeigen, falls hyperref nicht aktiv
}


\DefineBibliographyStrings{german}{
  andothers = {et\addabbrvspace al\adddot},
}
% Bibliografie-Driver anpassen
\DeclareBibliographyDriver{article}{%
  \printnames{author}%
  \addcomma\space
  \printfield{title}%
  \adddot\space
  \printfield[journaltitle]{journaltitle}%
  \space
  \printfield{volume}%
  \addcomma\space
  \printfield{pages}%
  \space
  (\printfield{year})%
  \adddot\space
  \iffieldundef{doi}
    {\printfield{url}}
    {\printfield{doi}}%
  \finentry
}
\DeclareBibliographyDriver{book}{%
  \printnames{author}%
  \addcomma\space
  \printfield{title}%
  \adddot\space
  \printlist{publisher}%
  \space
  \printlist{location}%
  \space
  (\printfield{year})%
  \adddot\space
  \iffieldundef{doi}
    {\printfield{url}~\printfield{isbn}}
    {\printfield{doi}}%
  \finentry
}
\DeclareNameAlias{sortname}{family-given}
\DeclareNameAlias{default}{family-given}

% Stelle sicher, dass \cite die Zahl in eckigen Klammern ausgibt
\DeclareCiteCommand{\cite}
  {\usebibmacro{prenote}}
  {\usebibmacro{citeindex}%
   \printtext[bibhyperref]{%
     [\printfield{labelnumber}] % Gibt die Nummer in eckigen Klammern aus
   }}
  {\multicitedelim}
  {\usebibmacro{postnote}}

% Ende Benes







% Last packages, do not change order or insert new packages after these ones
\usepackage[pdfusetitle, unicode, linkbordercolor=tugreen, citebordercolor=tugreen]{hyperref}
\usepackage{bookmark}
\usepackage[shortcuts]{extdash}

% Use package physics
\usepackage{physics}
\input{commands.tex}

%------------------------------------------------------------------------------
%-------------------------    Angaben zur Arbeit   ----------------------------
%------------------------------------------------------------------------------

\author{Julian Rosenbaum}
\title{Supraleitung auf dem Bienenwabengitter}
\date{2026}
\birthplace{Herdecke}
\chair{Lehrstuhl für Theoretische Physik I}
\division{Fakultät Physik}
\thesisclass{Bachelor of Science}
\submissiondate{12. August 2026}
\firstcorrector{Prof.~Dr.~Götz Uhrig}
\secondcorrector{Prof.~Dr.~Joachim Stolze}

% tu logo on top of the titlepage
\titlehead{\includegraphics[height=1.5cm]{logos/tu-logo.pdf}}

\begin{document}
\frontmatter
% \input{content/hints.tex}
\maketitle

% Gutachterseite
\makecorrectorpage

% hier beginnt der Vorspann, nummeriert in römischen Zahlen
\input{content/00_abstract.tex}
\tableofcontents

\mainmatter
% Hier beginnt der Inhalt mit Seite 1 in arabischen Ziffern
\input{content/01_einleitung.tex}
\input{content/02_grundlagen.tex}
\input{content/03_modell.tex}
\input{content/04_ergebnisse.tex}
\input{content/05_fazit.tex}
\appendix
% Hier beginnt der Anhang, nummeriert in lateinischen Buchstaben
\input{content/a_anhang.tex}
\input{content/b_anhang.tex}

\backmatter
\printbibliography

\cleardoublepage
% From https://www.tu-dortmund.de/studierende/im-studium/pruefungsangelegenheiten/allgemeine-vordrucke/
\includepdf{content/Eidesstattliche_Versicherung.pdf}

\end{document}
