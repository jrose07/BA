...
% Um die in~\autoref{sec:modell} hergeleiteten Selbstkonsistenzgleichungen~\eqref{eq:gap_eq_half_filling_integral} bzw.~\eqref{eq:gap_eq_arbitrary_filling_integral} zu lösen, wird
...
Für $U \rightarrow 0$ geht $\Delta_0$ asymptotisch gegen Null, während erkennbar ist, dass $\Delta_0$ im undotierten Fall tatsächlich erst oberhalb einer kritischen Kopplung $U_C$ von null verschieden wird.
...
Die lineare Regression liefert $A \approx \qty{6.6447}{W^{-2}}$.
...
\begin{figure}[htb]
    ...
    \caption{
        ...
        (c) und (d) vergleichen bei $E_D = \qty{12}{\milli W}$ die Abhängigkeiten von $U$ und $\mu$ von $\Delta_0$ (c) und kritischer Temperatur $T_C$ (d). In allen Teilabbildungen wird für die kritische Kopplung $U_C$ für $\mu = 0$ eingezeichnet.
    }
    \label{fig:ALLG_ABH}
\end{figure}
...
Tatsächliche \textit{ab initio} Rechnungen sind prinzipiell möglich, siehe dazu z.B.\ Refs.~\cite{giustino_electron-phonon_2017,morel_calculation_1962}, gehen aber weit über die BCS-Theorie hinaus und werden dementsprechend hier nicht betrachtet.
Es wird also auf den Vergleich mit experimentellen Daten zurückgegriffen. 
% Die Autoren in Ref.~\cite{ludbrook_evidence_2015} zeigen, dass in $\mathrm{Li}$-dotiertem Graphen eine Bandlücke von $\Delta = \qty{0.9(0.2)}{meV}$ bei $T=\qty{3.5}{\kelvin}$ gemessen wurde, 
Herrera et al.\ sagen in Ref.~\cite{herrera_topological_2024} bei $\mathrm{Tb}$-dotiertem Graphen anhand von experimentellen Daten mithilfe von numerischen Methoden eine kritische Temperatur von bis zu $T_C = \qty{600}{\milli\kelvin}$ voraus.
Es wird hierbei ungefähr bis zur VHS dotiert, was in dem hier verwendeten Modell einem chemischen Potential von $\mu \approx \qty{2.7}{eV}$ entsprechen würde.
Aufgrund dieser starken Dotierung beobachten die Autoren von Ref.~\cite{herrera_topological_2024} allerdings ein Abflachen der Energiebänder und dadurch eine vergrößerte VHS, was dazu führt, dass die VHS dort nur bei $\mu = \qty{1.55}{eV}$ liegt.
Etwaige Vergleiche mit dem Modell in dieser Arbeit sind also mit Vorsicht zu genießen.
% Des Weiteren kann $U$ im Vergleich zu der in Ref.~\cite{wehling_strength_2011} für das Hubbard-Modell in Graphen berechneten effektiven Coulombabstoßung $U_\text{rep} = \qty{9.3}{eV}$, betrachtet werden. 
% Da sich $U$ in der gleichen Größenordnung $1 - \qty{10}{eV}$ befinden sollte, können zumindest Kopplungen die weitaus über diesem Wert liegen ausgeschlossen werden.
Der Vergleich mit der im Hubbard-Modell für Graphen bestimmten effektiven Coulombabstoßung $U_\text{rep} = \qty{9.3}{eV}$ aus Ref.~\cite{wehling_strength_2011} liefert des Weiteren eine grobe obere Orientierung für die Energiskala von $U$. 
Kopplungen $U$, die deutlich oberhalb dieser Energieskala liegen, erscheinen damit zumindest als wenig plausibel.
...
Es wird zum einen überprüft, wie stark die Kopplung in Graphen sein müsste, damit bei halber Füllung bzw.\ schwacher Dotierung eine supraleitende Bandlücke entsteht. 
% Zum anderen soll eine Aussage darüber getätigt werden, bei welcher Dotierungsstärke $\mu$ aufgrund der ungefähren Einschränkung von $U$ Supraleitung erwartbar wäre und dabei besonders die VHS betrachtet werden. 
...
Abbildung~\ref{fig:TC_Graphen_color} zeigt die kritische Temperatur $T_C$ als Funktion von $U$ und $\mu$. 
...
%  Bereichs   auf die VHS und auf den kritischen Punkt $U_C$ dargestellt. 
Es ist zu erkennen, dass an der VHS bereits bei kleinen Kopplungen von ca. $\qty{1}{eV}$ verhältnismäßig große kritische Temperaturen auftreten, während dies bei halber Füllung erst ab einer kritischen Kopplung von $U_C \approx \qty{198}{eV}$ der Fall ist, was deutlich über dem realistischen Bereich von $U$ liegt. 
...
\begin{figure}[htb]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{}
    \end{subfigure}
    \caption{Kritische Temperatur $T_C$ in Abhängigkeit von Kopplung $U$ und Dotierungsstärke $\mU$. In Teilabbildung (a) ist eine Detailansicht des Bereichs um die VHS dargestellt und in (b) um den Bereich des quantenkritischen Punkts.}
    \label{fig:TC_Graphen_color}
\end{figure}

Etwas genauer wird der Bereich um die VHS in Abbildung~\ref{fig:U_VHS} dargestellt. 
Um einen Vergleich zu experimentellen Daten zu ziehen, könnte nun mit Ref.~\cite{herrera_topological_2024} verglichen werden.
Es ist zu erkennen, dass an der VHS die kritische Temperatur von $T_C = \qty{0.6}{K}$ in einem Bereich von $U \approx \num{0.5} - \qty{1.1}{eV}$ vorhergesagt wird, was dementsprechend damit ein realistischer Bereich wäre.
Es sei hier nur bemerkt, dass diese Einschränkung wie bereits erwähnt mit Vorsicht zu genießen ist und kleine Änderungen von $U$ nach oben bereits große Änderungen in $T_C$ bewirken.


% Geht man davon aus, dass in Ref.~\cite{herrera_topological_2024} tatsächlich in diesem $\mu$-Bereich gemessen wurde, kann $U$ auf diesen Bereich beschränkt und Supraleitung abseits der VHS verneint werden.
% mit Ref.~\cite{herrera_topological_2024} wäre $U$ damit auf diesen Bereich beschränkt und Supraleitung abseits der VHS wäre nur serh schwach. 
% Dies wäre also im Vergleich mit Ref.~\cite{herrera_topological_2024} unter Vernachlässigung der veränderten Bandstruktur eine 
\begin{figure}[htb]
    \centering
    ...
    \begin{subfigure}{0.38\textwidth}
        ...
        % \caption{}
    \end{subfigure}
    \caption{Kritische Temperatur $T_C$ in Abhängigkeit von $U$ um die VHS herum. Links ist der Gesamtverlauf zu sehen und rechts ein Zoom auf den Bereich bis maximal $T_C = \qty{1}{K}$.}
    \label{fig:U_VHS}
\end{figure}
