...
Diese werden so modifiziert, dass die Untergitterstruktur des Bienenwabengitters berücksichtigt und eine Verschiebung $\mu$ der Fermi-Energie zugelassen wird.
Der modifizierte Hamiltonian lautet damit 
\begin{align}
    \oH = \oHNN + \oH_{\mu} + \oHWW 
\end{align}
mit 
...
Dabei werden die allgemeinen fermionischen Operatoren $\ofed{}/\ofe{}$  durch die Untergitter-spezifischen Operatoren $\oad{}/\oa{}$ und $\obd{}/\ob{}$ ersetzt, die nur Elektronen auf dem jeweiligen Untergitter $A$ oder $B$ erzeugen, bzw.~vernichten. 
...
Die Summen werden hier mit $\sqrt{N}$ normiert, wobei $N$ auch hier wieder für die Anzahl an Gitterplätzen steht.
...
Ebenso kann begründet werden, dass aufgrund der makroskopischen Effekte bei der Bildung des Cooper-Paar-Kondensats sehr viele Teilchen beteiligt sind und dadurch quantenmechanische Fluktuation in guter Näherung vernachlässigt werden können~\cite[50]{tinkham_introduction_2004}.
...
Als Vorbereitung für die Näherung wird der quartische Term aus Gleichung~\eqref{eq:H_aMF} über das Wick-Theorem~\cite{wick_evaluation_1950,kehrein_flow_2006} wiefolgt umgeschrieben:
...
Im Folgenden wird diese allerdings weggelassen, da für die Normalordnung von bilinearen Termen $: \ofe{}\ofe{}: = \ofe{}\ofe{} - \expval{\ofe{} \ofe{}} $ gilt~\cite{kehrein_flow_2006} und $\expval{\ofe{}\ofe{}}$ einfach in die Konstante absorbiert werden kann.
...
Dies ist genau die bekannte Mean-Field-Näherung, die ein Vielteilchenproblem auf ein effektives Einteilchenproblem reduziert~\cite[66]{bruus_manybody_2004}.
...
Der durch den Hartree-Term entstehendende Beitrag ist also im Vergleich verschwindend gering.
...
Er enthält die anomale Erwartungswerte $\expval{\ocdu \ocdd}$ bzw. $\expval{\ocndd \ocndu}$, also Erwartungswerte der Paarerzeugung(-vernichtung) von Cooper-Paaren mit dem in \autoref{fig:s_wave} begründeten Gesamtimpuls $\vec{K} = 0$. 
Da diese Erwartungswerte die Teilchenzahlerhaltung verletzten, sind sie in der normalleitenden Phase des Systems immer null, für die supraleitende Phase sind sie allerdings aufgrund der makroskopischen Kohärenz der Cooper-Paare endlich, sie fungieren also als Ordnungsparameter der supraleitenden Phase.
...
\end{align}
kann der Hamiltonian in der kompakten Form
\begin{align}
...
Aufgrund der Untergittersymmetrie kann für die Ordnungsparameter die Unabhängigkeit des Untergitters, also $\Delta_{\oA} = \Delta_{\oB} \equiv \Delta$ angenommen werden.
...
% Für den Hartree-Term $\oH_1$ ergibt sich mit $\vk = \vk'$ 
% \begin{align}
%     \oH_1 &= -\frac{U}{N} \sum_\oC \sump{\vk} \left[\expval{\ocd{\vk, \up} \oc{\vk, \up}} \ocd{-\vk, \down} \ocndd + \expval{\ocd{-\vk, \down} \ocndd} \ocd{\vk, \up} \ocndu\right]
% \end{align}
...
\end{align}
eingeführt um $\oHeff$ in Matrix-Form 
\begin{align}
...
\section{Selbstkonsistenz bei halber Füllung}
...
Wie bereits in \autoref{subsec:Bienenwabengitter} argumentiert, liegt halbe Füllung genau dann vor, wenn alle Zustände bis zur Energie $E=0$ gefüllt sind, also die Fermienergie verschwindet.
...
\end{align}
und mit der zugehörigen unitären Trafo-Matrix\footnote{Es sei hier nur erwähnt, dass bei der Bestimmung der unitären Matrix $\dul{U}$ aufgrund der entarteten Eigenwerte $E_\vk$ darauf geachtet werden muss, dass die berechneten Eigenvektoren orthogonal zueinander sind. Ist dies nicht der Fall muss gegebenfalls ein Gram-Schmidt-Verfahren durchgeführt werden um die Trafo-Matrix unitär zu machen.}
\begin{align}
...
Die Energiebänder des Bienenwabengitters im normalleitenden Grundzustand werden für $\Delta \rightarrow 0$ zurückgewonnen und ergeben sich zu
\begin{align}
    \epsilon_{\vk} = \pm \abs{f(\vk)} = \pm t \sqrt{3 + 2 \cos(\sqrt{3} k_y a) + 4 \cos(\sqrt{3} k_y a/2) \cos(3 k_x a/2)}~\label{eq:dispersion} \, ,
\end{align}
dessen Eigenschaften in \autoref{subsec:Bienenwabengitter} qualitativ genauer diskutiert werden.
...
Dies ist die typische BCS-Gap-Gleichung für den Ordnungsparameter $\Delta$~\cite{czycholl_theoretische_2017}. 
...
Der Beitrag zur Supraleitung ist damit für beide Energiebänder gleich groß, es muss nicht zwischen Ihnen unterschieden werden, effektiv ergibt sich also nur die Gleichung wie für ein Band.
% \begin{align}
...
\end{align}
umgeschrieben. 
Dabei wird der zuvor in der Schreibweise $\sump{}$ implizit dargestellte Energiecutoff $\hbar \omega_D$ nun wieder explizit berücksichtigt.
...
Bei der Formulierung der Selbstkonsistenz über die Zustandsdichte muss nun also auf formell auf die Beiträge der unterschiedlichen Energiebänder geachtet werden, also gilt 
\begin{align}
