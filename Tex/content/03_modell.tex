...
Dazu wird der quartische Term aus Gleichung~\eqref{eq:H_vor_MF} wie folgt umgeschrieben~\cite{ring_nuclear_1980}:
...
\end{align}
% Neben der Hamilton-Matrix $\dul{h}$ werden ebenfalls die Nambu-Spinoren $\vopsi$ und $\vopsid$ über 
...
% mit den Quasiteilchenenergien $E_\vk$
...
\end{align}
und mit~\eqref{eq:Delta_rho} folgt 
\begin{align}
    ...
    &= \frac{U \Delta}{4N} \sump{\vk} \left[\frac{1}{E_+} \left(F(-E_+) - F(E_+)\right) + \frac{1}{E_-} \left(F(-E_-) - F(E_-)\right) \right] \\ \notag \\
