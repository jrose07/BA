...
\begin{align}
    E_\text{F} - \hbar \omega_{\text{D}} \leq \epsilon_\vk \leq E_\text{F} + \hbar \omega_\text{D}
\end{align}
null ist.
...
Der Wechselwirkungsterm beschreibt jetzt im Vergleich mit dem BCS-Hamiltonian~\eqref{eq:BCS} die Bildung von Cooper-Paaren mit Singulett-Kopplung aber dafür endlichem Gesamtimpuls $\vec{K} = \vk + \vk'$ und hat mit dem zuvor implizit formulierte Cutoff über die Einteilchenenergie $\epsilon_\vk$ nun die explizite Form
\begin{align}
    \oHWW &= - \frac{U}{N} \sum_{\vk, \vk', \vq} \sum_\oC \ocdu \ocdd \ocndd \ocndu \Theta(\hbar \omega_D - \abs{\epsilon_\vk}) \Theta(\hbar \omega_D - \abs{\epsilon_{\vk'}}) \, .
\end{align}
% wobei hier der zuvor implizit formulierte Cutoff über die Einteilchenenergie $\epsilon_\vk$ nun explizit formuliert ist.
In dem Rest der Arbeit wird die Schreibweise 
\begin{align}
    \sump{\vk} &= \sum_{\vk} \Theta(\hbar \omega_D - \abs{\epsilon_\vk})
\end{align}
...
Um im folgenden den Ordnungsparameter $\Delta$ bestimmen zu können, müssen die anomalen Erwartungswerte aus der Definition berechnet werden.
